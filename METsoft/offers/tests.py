...
class OffersIntegrationTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        # offers
        cls.of_stat_id1 = models.OfferStatus.objects.create(
            id=1, offer_status="test status id 1"
        )
        cls.of_stat_id2 = models.OfferStatus.objects.create(
            id=2, offer_status="test status id 2"
        )
        ...
        # materials
        cls.content_type_offers_material = ContentType.objects.get(
            app_label="offers", model="material"
        )
        cls.perm_add_material = Permission.objects.get(
            codename="add_material", content_type=cls.content_type_offers_material
        )
        cls.perm_change_material = Permission.objects.get(
            codename="change_material", content_type_id=cls.content_type_offers_material
        )
        models.MaterialGroup.objects.create(
            id=1, mat_group=21, description="test_material_group"
        )
        ...
        cls.heat_treat1 = models.HeatTreatment.objects.create(
            term="test_heat_treatment_1"
        )
        cls.pat_taper_1 = models.PatternTaper.objects.create(
            taper="test_pattern_taper_1"
        )
        cls.atest_type1 = models.AtestType.objects.create(atest="test_atest_type_1")
        cls.of_pat_status1 = models.OfferPatternStatus.objects.create(
            status="offer_pattern_status_1"
        )
    ...
    def test_offer_str(self):
        self.assertEqual(
            self.offer1.__str__(),
            "{} -- {} -- {}".format(
                self.offer1.offer_no, self.offer1.client, self.offer1.positions_amount
            ),
        )
    ...
    def test_detail_str(self):
        self.assertEqual(
            ...
            "{} {} {}".format(
                self.detail1.cast_name,
                self.detail1.drawing_no,
                self.detail1.cast_weight,
            ),
        )
    ...
    def test_url_offer_details(self):
        ...
        response = self.client.post(
            reverse("offer-details", args=[self.offer1.pk]), {"status": "1"}
        )
        self.assertEqual(response.status_code, 200)
        response = self.client.post(
    ...
    def test_url_offer_notices_post(self):
        self.user1_logged()
        response = self.client.post(
            reverse("notices"), {"content": "new_notices"}, follow=True
        )
    ...
    def test_url_offer_detail_create(self):
        ...
        response = self.client.get(
            reverse("detail-create", args=[self.offer1.pk]), {"steel": ""}
        )
        self.assertEqual(response.status_code, 403)
        ...
        self.assertEqual(response.status_code, 200)
        response = self.client.get(
            reverse("detail-create", args=[self.offer1.pk]), {"iron": ""}
        )
        self.assertEqual(response.status_code, 200)
        response = self.client.post(
            reverse("detail-create", args=[self.offer1.pk]),
    ...
    def test_url_offer_detail_update(self):
        ...
        self.assertEqual(
            ...
            [(reverse("offer-details", args=[self.offer1.pk]), 302)],
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_url_offer_detail_delete(self):
        self.user1_logged()
        response = self.client.get(
            reverse("detail-delete", args=[self.offer1.pk, self.detail1.pk])
        )
        self.assertEqual(response.status_code, 403)
