...
class DetailViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    queryset = Detail.objects.exclude(offer__status__in=[1]).order_by(
        "-id"
    )  # without "W opracowaniu"
...
class DetailSearchingView(FormView):
    ...
    def post(self, request, *args, **kwargs):
        if (
            request.POST["cast_name"]
            or request.POST["drawing_no"]
            or request.POST["offer_no"]
        ):
            ...
            return render(
                request, "offers/detail_searching_results.html", {"objects": objects}
            )
...
class OffersStatisticsView(LoginRequiredMixin, PermissionRequiredMixin, FormView):
    ...
    def gen_report_offer_per_statuses(offers):
        ...
        for index, status in enumerate(offer_statuses):
            statuses_stats.append(
                {"status": status.offer_status, "amount": 0,}
            )
            for offer in offers:
    ...
    def gen_report_details_per_mat_group(details):
        ...
        for index, mat_group in enumerate(mat_groups):
            detail_stats.append(
                {"mat_group": mat_group.mat_group, "amount": 0,}
            )
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if request.method == "POST":
            ...
            form = forms.OfferStatsForm(
                initial={
                    "date_stats_from": date_stats_from,
                    "date_stats_to": date_stats_to,
                }
            )

        offers = Offer.objects.filter(
            date_tech_out__gt=date_stats_from, date_tech_out__lt=date_stats_to
        )
        details = Detail.objects.filter(
            offer__date_tech_out__gt=date_stats_from,
            offer__date_tech_out__lt=date_stats_to,
        )
