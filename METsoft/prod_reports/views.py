...
class NonDestructiveTestingViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    queryset = Operation.objects.filter(
        opdict__in=[10, 21, 22, 24, 25, 26, 28, 56], completion_date1__isnull=False
    )
    serializer_class = OperationSerializer
...
class InsertedDataList(ListView):
    ...
    queryset = (
        ...
        .annotate(
            ...
            cast_weight=Max(
                "parameter_value1", filter=Q(opdict_id=51) or Q(opdict_id=43)
            ),
...
class MachiningViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    queryset = Operation.objects.filter(
        opdict__in=[19, 20, 64, 65, 82, 91], completion_date1__isnull=False
    )
    serializer_class = OperationSerializer
...
class MonitoringInWorkViewSet(viewsets.ReadOnlyModelViewSet):
    queryset = Cast.monitoring().filter(
        cast_pcs__gt=F("sent") + F("cancelled") + F("finished")
    )
    serializer_class = MonitoringSerializer
...
class WeightPerClientView(TemplateView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context["objects"] = (
            Cast.objects.filter(cast_status__in=[1, 2, 3, 7])
            .values("customer")
            .annotate(
        ...
        context["sums"] = Cast.objects.filter(cast_status__in=[1, 2, 3, 7]).aggregate(
            new=Sum("cast_weight", filter=Q(cast_status=1)),
            planned=Sum("cast_weight", filter=Q(cast_status=7)),
            poured=Sum("cast_weight", filter=Q(cast_status=2)),
            on_stock=Sum("cast_weight", filter=Q(cast_status=3)),
        ...
        if met_number or company or cast_name or picture_number:
            casts = (
                ...
                .annotate(
                    ...
                    casting_weight=Max(
                        "parameter_value1", filter=Q(opdict_id__in=[43, 51])
                    ),
            ...
            return render(
                request, "prod_reports/execution_time_results.html", {"objects": casts}
            )
