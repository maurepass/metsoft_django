...
class Pattern(models.Model):
    ...
    drawing_number = models.CharField(
        "Numer rysunku", max_length=100, blank=True, null=True
    )
    pattern_name = models.CharField(
        "Nazwa odlewu", max_length=100, blank=True, null=True
    )
    last_order = models.DateField(
        "Ostatnie zlecenie", blank=True, null=True, default=date.today
    )
    orders_amount = models.IntegerField(
        "Ilość zleceń", blank=True, null=True, default=1
    )
    area = models.FloatField("Powierzchnia", blank=True, null=True)
    layer_number = models.CharField(
        "Numer ułożenia", max_length=100, blank=True, null=True
    )
    layer_place = models.CharField(
        "Miejsce ułożenia", max_length=100, blank=True, null=True
    )
    material = models.CharField("Materiał", max_length=30, blank=True, null=True)
    cart_number = models.CharField(
        "Numer kartoteki", max_length=30, blank=True, null=True
    )
    pattern_index = models.CharField(
        "Numer indexu modelu", max_length=100, blank=True, null=True
    )
    ...
    verification_date = models.CharField(
        "Data weryfikacji", max_length=30, blank=True, null=True, default=date.today
    )
    ...
    move_in = models.DateField(
        "Data zmiany statusu", blank=True, null=True, default=date.today
    )
    ...
    def add_status_to_pattern_history(self):
        PatternHistory.objects.create(
            pattern=self, status=self.status, date=self.move_in
        )
