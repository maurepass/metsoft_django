...
class PatternReportForm(forms.Form):
    ...
    customer1 = forms.CharField(max_length=50,
                                required=True,
...
class PatternCreateForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'verification_date': forms.DateInput(attrs={'id': 'datepicker2', 'width': '230px'}, format='%Y-%m-%d'),
            'remarks': forms.Textarea(attrs={'rows': 1, 'cols': 50}),
            'move_in': forms.DateInput(attrs={'id': 'datepicker1', 'width': '230px'}, format='%Y-%m-%d'),
            'last_order': forms.DateInput(attrs={'id': 'datepicker3', 'width': '230px'}, format='%Y-%m-%d'),
        }
