...
class OrdersView(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if last_poc_kokila.pk > last_poc_orders.id_poc:
            items = Pocastord.objects.using("kokila").filter(
                pk__gt=last_poc_orders.id_poc - 50
            )
            for item in items:
...
class WZTDailyReport(TemplateView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context["nonconformities"] = Operation.objects.filter(
            accordance=3, completion_date1=completion_date
        )
